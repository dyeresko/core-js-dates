...
function getNextFriday(date) {
  ...
  } while (getDayName(fridayDay) !== 'Friday');
...
function formatDate(date) {
  ...
  const dateChunk1 = `${month}/${day}/${year}`;
  const noon = dateToFormat.getUTCHours() >= 12 ? 'PM' : 'AM';
  const hours =
  ...
  const dateChunk2 = `${hours}:${minutes < 10 ? '0' : ''}${minutes}:${seconds < 10 ? '0' : ''}${seconds} ${noon}`;
  return `${dateChunk1}, ${dateChunk2}`;
}
...
function getQuarter(date) {
  const firstQuarter = new Date(date.getFullYear(), 2, 31);
  const secondQuarter = new Date(date.getFullYear(), 5, 30);
  const thirdQuarter = new Date(date.getFullYear(), 8, 30);
  const fourthQuarter = new Date(date.getFullYear(), 11, 31);
  if (date.getTime() < firstQuarter.getTime()) {
    return 1;
  }
  if (date.getTime() < secondQuarter.getTime()) {
    return 2;
  }
  if (date.getTime() < thirdQuarter.getTime()) {
    return 3;
  }
  if (date.getTime() < fourthQuarter.getTime()) {
    return 4;
  }
  return -1;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const splittedStart = period.start.split('-');
  const splittedEnd = period.end.split('-');
  const start = new Date(
    splittedStart[2],
    splittedStart[1] - 1,
    splittedStart[0]
  );
  const end = new Date(splittedEnd[2], splittedEnd[1] - 1, splittedEnd[0]);
  const array = [];
  do {
